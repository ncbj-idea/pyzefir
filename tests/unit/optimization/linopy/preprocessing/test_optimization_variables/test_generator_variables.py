...
def test_generator_variables(
    ...
    assert variables.cap_plus.size == n_gens * yy
    assert all(
        len(variables.cap_plus.indexes["index"][i]) == 2 for i in range(n_gens * yy)
    )
    assert variables.cap_minus.size == n_gens * yy * yy
    assert all(
        len(variables.cap_minus.indexes["index"][i]) == 3 for i in range(n_gens * yy)
    )
    assert variables.cap_base_minus.size == n_gens * yy
    assert all(
        len(variables.cap_base_minus.indexes["index"][i]) == 2
        for i in range(n_gens * yy)
    )
