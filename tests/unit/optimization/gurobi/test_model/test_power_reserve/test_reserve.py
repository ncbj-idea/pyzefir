...
def test_reserves(
    ...
    gen_et = engine.results.generators_results.gen_et
    gens_of_tag = invert_dict_of_sets(engine.parameters.gen.tags)
    gens_of_tag_str = {
...
def _to_test_reserve(
    ...
    gens_of_tag_str: dict[int, set[int]],
) -> None:
    for energy_type, tag_to_reserve in power_reserves.items():
        for tag, reserve in tag_to_reserve.items():
            cap_sum = sum(
                np.asarray(cap[str(gen_name)][0]) for gen_name in gens_of_tag_str[tag]
            )
            gen_sum = sum(
                np.asarray(gen_et[str(gen_name)][energy_type])
