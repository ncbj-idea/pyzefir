...
def test_local_supplementary_capacity_upper_bound_constraints(
    ...
    assert np.allclose(res_frac["lbs_ee"][0], np.array([1, 0, 0, 0, 0]))
    assert np.allclose(res_frac["lbs_ee2"][0], np.array([0, 1, 1, 1, 1]))
    ...
    cap_vals = set(res_cap["local_pv"][0])
    cap2_vals = set(res_cap["local_pv2"][0])
    resulted_tcap_plus = np.array(
        engine.results.generators_results.tcap_plus["aggr_ee"]["pv"][0]
    )
    test_data_check = (
        len(cap_vals) == 2
        and len(cap2_vals) == 2
