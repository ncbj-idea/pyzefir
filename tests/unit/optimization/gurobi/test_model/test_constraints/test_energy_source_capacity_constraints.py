...
from pyzefir.optimization.gurobi.model import OptimizationError
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            {
                EE: pd.Series([0] + [10000000] * (N_YEARS - 1)),
...
def test_generator_capacity_constraints(
    ...
    for name, expected in excepted_capacity.items():
        assert np.allclose(
            engine.results.generators_results.cap[name][0].to_list(), expected
        )
